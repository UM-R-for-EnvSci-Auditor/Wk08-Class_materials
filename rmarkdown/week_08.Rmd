...
That disnt quite work. You can specify what format it is in.
...
But what if we need date AND time. R has two speciffic formats for that:
...
We will work with the chemistry data later, as it is easier to del with once we have combined the different batches into one.
...
equipment_malfunction <- interval(ymd("2018/02/15"), ymd("2018/03/20"))
...
results <- bind_rows(lst(batch_1, batch_2, batch_3, batch_4), .id = "batch")
...
ggplot() +
  ...
  geom_smooth(aes(x = sampling_date, y = concentration, colour = compound)) +
  theme_pepe()
